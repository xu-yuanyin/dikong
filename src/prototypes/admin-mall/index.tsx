...
import { EyeOutlined, SearchOutlined, CheckCircleOutlined, ShopOutlined, AuditOutlined, CloseCircleOutlined } from '@ant-design/icons';
...
var GOODS_DATA = [
  { key: '1', id: 'MALL-2026-101', title: '大疆 DJI Mavic 3 Enterprise', category: '工业级无人机', scenario: '航拍测绘', price: '¥22,000.00', provider: 'XX无人机专营店', time: '2026-04-18 10:00:00', status: 'normal', stock: 50, specs: { weight: '915g', battery: '45分钟', payload: '无', range: '32公里' }, desc: 'DJI Mavic 3 Enterprise 配备机械快门、56倍变焦相机及 RTK 模块。' },
  { key: '2', id: 'MALL-2026-102', title: '纵横 CW-15 垂直起降固定翼', category: '工业级无人机', scenario: '巡检安防', price: '电议', provider: '纵横官方旗舰店', time: '2026-04-19 14:30:22', status: 'normal', stock: 5, specs: { weight: '14.5kg', battery: '180分钟', payload: '3kg', range: '100公里' }, desc: 'CW-15 采用先进的垂直起降技术。' },
  { key: '3', id: 'MALL-2026-104', title: '大疆 DJI Air 3S 旗舰航拍机', category: '工业级无人机', scenario: '航拍测绘', price: '¥8,999.00', provider: '张明', time: '2026-05-18 14:00:00', status: 'pending', stock: 30, specs: { weight: '720g', battery: '46分钟', payload: '无', range: '20公里' }, desc: '全新 Air 3S 搭载 1 英寸 CMOS 传感器。' },
  { key: '4', id: 'MALL-2026-105', title: '无人机专用降落伞安全系统', category: '安全设备', scenario: '通用/其他', price: '¥3,200.00', provider: '张明', time: '2026-05-19 11:00:00', status: 'pending', stock: 50, specs: { weight: '350g', battery: '-', payload: '-', range: '-' }, desc: '适配 25kg 以下多旋翼无人机的紧急降落伞系统。' },
...
var Component = function AdminMallPage() {
  ...
  var handleApprove = function () { message.success('审核通过！该商品已上架展示。'); setAuditOpen(false); };
  var handleReject = function () { if (!rejectReason.trim()) { message.warning('请输入驳回原因'); return; } message.success('已驳回该商品上架申请。'); setAuditOpen(false); setRejectReason(''); };
  ...
  var tabItems = [
    ...
    { key: 'pending', label: '待审核 (' + GOODS_DATA.filter(function (d) { return d.status === 'pending'; }).length + ')' },
    { key: 'normal', label: '已通过 (' + GOODS_DATA.filter(function (d) { return d.status === 'normal'; }).length + ')' },
    { key: 'rejected', label: '已驳回 (' + GOODS_DATA.filter(function (d) { return d.status === 'rejected'; }).length + ')' }
  ];
  var filteredData = activeTab === 'all' ? GOODS_DATA : GOODS_DATA.filter(function (d) { return d.status === activeTab; });
  ...
  return (
    <AdminLayout activeKey="admin-mall">
      ...
      <Modal title="商品详情" open={viewOpen} onCancel={function () { setViewOpen(false); }} width={800} footer={<Button onClick={function () { setViewOpen(false); }}>关闭</Button>}>
        {currentRecord && (
      ...
      <Modal title={<span style={{ color: '#722ed1' }}><AuditOutlined style={{ marginRight: 8 }} />商品上架审核</span>} open={auditOpen} onCancel={function () { setAuditOpen(false); setRejectReason(''); }} width={640}
        footer={[<Button key="cancel" onClick={function () { setAuditOpen(false); }}>取消</Button>, <Button key="reject" danger onClick={handleReject} icon={<CloseCircleOutlined />}>驳回上架</Button>, <Popconfirm key="approve" title="确认审核通过？" onConfirm={handleApprove}><Button type="primary" style={{ background: '#52c41a', borderColor: '#52c41a' }} icon={<CheckCircleOutlined />}>同意上架</Button></Popconfirm>]}>
        {currentRecord && (<div>
          <div style={{ padding: 16, background: '#fafafa', borderRadius: 8, marginBottom: 16 }}>
            <Descriptions column={2} size="small">
              ...
              <Descriptions.Item label="商品名称" span={2}><span style={{ fontWeight: 600 }}>{currentRecord.title}</span></Descriptions.Item>
              <Descriptions.Item label="发布商户">{currentRecord.provider}</Descriptions.Item>
              <Descriptions.Item label="分类"><Tag color="purple">{currentRecord.category}</Tag></Descriptions.Item>
              <Descriptions.Item label="售价">{currentRecord.price}</Descriptions.Item>
              <Descriptions.Item label="库存">{currentRecord.stock} 件</Descriptions.Item>
            </Descriptions>
          </div>
