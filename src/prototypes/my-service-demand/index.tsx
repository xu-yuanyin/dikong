...
var DEMAND_DATA = [
  ...
  { key: '2', title: '航拍服务需求（房地产项目）', type: '低空旅游', budget: '¥5,000-8,000', area: '全市', status: '已下线', createDate: '2026-04-10', deadline: '2026-04-30', desc: '楼盘宣传片航拍素材收集' },
  { key: '3', title: '测试需求发包', type: '飞行器服务', budget: '电议', area: '郊区', status: '已下线', createDate: '2026-04-18', deadline: '2026-05-20', desc: '测试使用，无需响应' },
  { key: '4', title: '电力巡检服务外包', type: '飞行器服务', budget: '¥10,000-15,000', area: '全市', status: '已下线', createDate: '2026-03-15', deadline: '2026-04-15', desc: '高压线网日常巡检' },
...
var COLUMNS = [
  ...
  { title: '状态', dataIndex: 'status', key: 'status', render: function (s: string) { return <Tag color={s === '征集中' ? 'green' : 'default'}>{s}</Tag>; } },
  { title: '发布日期', dataIndex: 'createDate', key: 'createDate' },
  { title: '操作', key: 'action', render: function (_: any, record: any) {
    return (
      <div style={{ display: 'flex', gap: 8 }}>
        <a onClick={function () { handleNavigate('service-demand-detail'); }}>查看</a>
        {record.status === '征集中' && <a onClick={function () { message.success('已下线需求'); }}>下线</a>}
            ...
            <Card title="我的服务需求" extra={<Button type="primary" icon={<PlusOutlined />} onClick={function () { handleNavigate('demand-publish'); }}>发布需求</Button>} style={{ borderRadius: 12 }}>
              <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
                <div style={{ padding: '12px 20px', background: '#f6ffed', borderRadius: 8, textAlign: 'center' }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#52c41a' }}>{DEMAND_DATA.filter(function (d) { return d.status === '征集中'; }).length}</div>
                  <div style={{ fontSize: 12, color: '#8c8c8c' }}>征集中</div>
                </div>
