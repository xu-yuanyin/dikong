...
import { Card, Tag, Button, Modal, Form, Input, Select, Avatar, Descriptions, Divider, message, Empty, Breadcrumb } from 'antd';
import { UserOutlined, RocketOutlined, TeamOutlined, CustomerServiceOutlined, ShopOutlined, BankOutlined, PlusOutlined, SwapOutlined, CheckCircleOutlined, ClockCircleOutlined, ExclamationCircleOutlined, SafetyCertificateOutlined, HomeOutlined, IdcardOutlined, PhoneOutlined, EnvironmentOutlined } from '@ant-design/icons';
...
var MY_ROLES = [
  ...
  { value: 'government', label: '政府部门', color: '#1677ff', icon: <BankOutlined />, status: 'certified', certTime: '2026-05-01', isCurrent: false },
  ...
  { value: 'merchant', label: '商户', color: '#fa8c16', icon: <ShopOutlined />, status: 'certified', certTime: '2026-04-10', isCurrent: false },
  { value: 'personal', label: '个人用户', color: '#1677ff', icon: <UserOutlined />, status: 'certified', certTime: '2026-01-10', isCurrent: false },
...
var Component = function RoleManagementPage() {
  ...
  var [switchTarget, setSwitchTarget] = useState<typeof MY_ROLES[0] | null>(null);
  ...
  var handleSwitch = function (role: typeof MY_ROLES[0]) {
    if (role.status !== 'certified') {
      message.warning('该角色尚未认证通过，无法切换');
      return;
    }
    ...
    if (switchTarget) {
      setMyRoles(function (prev) {
        return prev.map(function (r) {
          return Object.assign({}, r, { isCurrent: r.value === switchTarget.value });
        });
      });
  ...
  var renderCertForm = function () {
    ...
    if (selectedRole === 'merchant') {
      return (
        <>
          {enterpriseFields}
          <Form.Item name="productCategories" label="经营品类" rules={[{ required: true, message: '请选择' }]}>
            <Select size="large" mode="multiple" placeholder="请选择经营品类（可多选）" options={PRODUCT_CATEGORIES} />
          </Form.Item>
        </>
      );
    }
      ...
      return (
        <>
          <Form.Item name="deptName" label="部门名称" rules={[{ required: true, message: '请输入' }]}>
            <Input size="large" placeholder="请输入政府部门全称" />
          </Form.Item>
          <Form.Item name="deptCode" label="机构代码" rules={[{ required: true, message: '请输入' }]}>
            <Input size="large" placeholder="请输入统一机构代码" />
          </Form.Item>
          <Form.Item name="contactName" label="联系人" rules={[{ required: true, message: '请输入' }]}>
            <Input size="large" prefix={<UserOutlined />} placeholder="请输入联系人姓名" />
          </Form.Item>
      ...
      <header style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)', padding: '0 24px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56 }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <a style={{ color: 'rgba(255,255,255,0.65)', cursor: 'pointer', fontSize: 14 }} onClick={function () { handleNavigate('home'); }}>首页</a>
            <a style={{ color: 'rgba(255,255,255,0.65)', cursor: 'pointer', fontSize: 14 }} onClick={function () { handleNavigate('profile-certified'); }}>个人中心</a>
          </div>
        </div>
      </header>

      <div style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
        <Breadcrumb items={[{ title: <span onClick={function () { handleNavigate('home'); }} style={{ cursor: 'pointer' }}>首页</span> }, { title: <span onClick={function () { handleNavigate('profile-certified'); }} style={{ cursor: 'pointer' }}>个人中心</span> }, { title: '角色管理' }]} style={{ marginBottom: 16 }} />
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        {!role.isCurrent && role.status === 'certified' && (
                          <Button icon={<SwapOutlined />} onClick={function () { handleSwitch(role); }}>切换角色</Button>
                        )}
                        {role.status === 'pending' && (
                          <Button type="link" disabled>审核中</Button>
                        )}
                      </div>
                    </div>
                  </Card>
